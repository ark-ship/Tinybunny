...
export default function TinyBunnyFinal() {
  ...
  // FETCH SUPPLY - Paksa update lebih sering
  const fetchSupply = useCallback(async () => {
    try {
      ...
      console.log("Sync Supply:", Number(count));
    } catch (e) { 
  ...
  const connectWallet = async () => {
    if (typeof window !== 'undefined' && window.ethereum) {
      try {
        ...
        } catch (err) { console.log("Network switch suggested."); }
  ...
  const handleMint = async () => {
    ...
    try {
      ...
      console.error(err);
      alert(err.reason || "Mint failed. Cek saldo atau network lo.");
    } finally {
      setIsMinting(false);
    }
  };
    ...
    <main className="min-h-screen text-black overflow-x-hidden relative" 
      ...
      <nav className="fixed w-full top-0 bg-white/40 backdrop-blur-md border-b-4 border-black z-50">
        <div className="max-w-6xl mx-auto px-6 h-16 flex justify-between items-center">
            ...
            <button 
              ...
              className="text-[8px] font-bold bg-white border-2 border-black px-4 py-2 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:shadow-none"
            >
      ...
      <div className="pt-32 pb-12 px-6 max-w-6xl mx-auto grid lg:grid-cols-2 gap-12 items-center relative z-10">
        <div className="space-y-8">
            <div className="inline-block border-2 border-black px-3 py-1 text-[8px] font-bold bg-yellow-300 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] uppercase">MegaETH Mainnet</div>
            ...
            <div className="bg-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] max-w-sm space-y-6">
                <div className="flex items-center justify-between border-b-2 border-black pb-4">
                    ...
                    <div className="text-[10px] font-black italic text-pink-600 tracking-tighter">0.0002 ETH</div>
                </div>
                ...
                <div className="flex justify-between text-[8px] font-bold tracking-tighter">
